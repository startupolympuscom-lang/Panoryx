import type { LucideIcon } from "lucide-react";
import {
  LayoutDashboard,
  Fuel,
  Clock,
  Gauge,
  Container,
  Truck,
  Wallet,
  Building2,
  Users,
  Wrench,
  BarChart3,
  Settings,
  ShoppingBag,
  CreditCard,
  Landmark,
  Coffee,
  Wallet2,
  Ruler,
  IdCard,
  FileText,
  Ticket,
  Banknote,
  Droplet,
  Lock,
  Globe,
  Share2,
  MessageSquare,
} from "lucide-react";

export interface NavItem {
  href: string;
  label: string;
  icon: LucideIcon;
}

export const panoStationNavItems: NavItem[] = [
  { href: "/app/panostation", label: "Vue d'ensemble", icon: LayoutDashboard },
  { href: "/app/panostation/ventes", label: "Ventes", icon: Fuel },
  { href: "/app/panostation/quarts", label: "Quarts", icon: Clock },
  { href: "/app/panostation/pompes", label: "Pompes et compteurs", icon: Gauge },
  { href: "/app/panostation/cuves", label: "Cuves et stocks", icon: Container },
  { href: "/app/panostation/veille-prix", label: "Veille prix mondiaux", icon: Globe },
  { href: "/app/panostation/livraisons", label: "Livraisons", icon: Truck },
  { href: "/app/panostation/jauge", label: "La Jauge", icon: Ruler },
  { href: "/app/panostation/livreurs", label: "Livreurs", icon: IdCard },
  { href: "/app/panostation/documents", label: "Documents", icon: FileText },
  { href: "/app/panostation/caisse", label: "Caisse", icon: Wallet },
  { href: "/app/panostation/boutique", label: "Boutique", icon: ShoppingBag },
  { href: "/app/panostation/lubrifiants", label: "Lubrifiants", icon: Droplet },
  { href: "/app/panostation/credits", label: "Crédits clients", icon: CreditCard },
  { href: "/app/panostation/bons", label: "Bons et vignettes", icon: Ticket },
  { href: "/app/panostation/cheques", label: "Chèques", icon: Banknote },
  { href: "/app/panostation/banque", label: "Rapprochement bancaire", icon: Landmark },
  { href: "/app/panostation/cafe", label: "Café / Restaurant", icon: Coffee },
  { href: "/app/panostation/fournisseurs", label: "Fournisseurs", icon: Building2 },
  { href: "/app/panostation/equipe", label: "Équipe", icon: Users },
  { href: "/app/panostation/salaires", label: "Salaires", icon: Wallet2 },
  { href: "/app/panostation/maintenance", label: "Maintenance et incidents", icon: Wrench },
  { href: "/app/panostation/marketing", label: "Marketing réseaux sociaux", icon: Share2 },
  { href: "/app/panostation/messagerie", label: "Messagerie interne", icon: MessageSquare },
  { href: "/app/panostation/rapports", label: "Rapports", icon: BarChart3 },
  { href: "/app/panostation/parametres-comptables", label: "Paramètres comptables", icon: Lock },
  { href: "/app/panostation/parametres", label: "Paramètres", icon: Settings },
];

export const fuelTypeLabels: Record<string, string> = {
  gasoil: "Gasoil",
  sp95: "Sans Plomb 95",
  sp98: "Sans Plomb 98",
};
